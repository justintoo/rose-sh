...
print >> sys.stdout, "[INFO] ROSESH_FLAGS=%s" % (flags)
...
# --no-object-files
#--------------------------------------
parser.add_argument('--object-files', action="store_true", dest='object_files', help="Detect .o object files as filenames for processing")
parser.add_argument('--no-object-files', action="store_false", dest='object_files', help="Detect .o object files as filenames for processing")
parser.set_defaults(object_files=False)
...
if options.tool:
  # Don't pass this option on to the tool:
  # See (1) http://stackoverflow.com/questions/35733262/is-there-any-way-to-instruct-argparse-python-2-7-to-remove-found-arguments-fro
  # See (2) https://docs.python.org/2/library/argparse.html#partial-parsing
  sys.argv = sys.argv[:1] + unknown
  tool = options.tool
else:
  print >> sys.stderr, "[FATAL] --tool <toolname> was not specified"
...
try:
  ...
  if retcode == 1:
    ...
    print >> sys.stdout, 'Command returned %d: %s', (retcode, cmd)
