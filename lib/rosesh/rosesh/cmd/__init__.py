...
import rosesh
import rosesh.config
import spack.spec

command_path = os.path.join(rosesh.lib_path, "rosesh", "cmd")
...
def parse_specs(args, **kwargs):
    ...
    except rosesh.parse.ParseError as e:
        tty.error(e.message, e.string, e.pos * " " + "^")
        sys.exit(1)

    except rosesh.spec.SpecError as e:
        tty.error(e.message)
        sys.exit(1)
...
def disambiguate_spec(spec):
    matching_specs = rosesh.store.db.query(spec)
...
def display_specs(specs, **kwargs):
    ...
    for i, (architecture, compiler) in enumerate(sorted(index)):
        ...
        header = "%s{%s} / %s{%s}" % (rosesh.spec.architecture_color,
                                      architecture, rosesh.spec.compiler_color,
                                      compiler)
