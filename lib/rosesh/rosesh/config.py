"""This module implements ROSE-SH's configuration file handling.

This implements ROSE-SH's configuration system, which handles merging
multiple scopes with different levels of precedence.  See the
documentation on :ref:`configuration-scopes` for details on how ROSE-SH's
...
When read in, ROSE-SH validates configurations with jsonschemas.  The
schemas are in submodules of :py:mod:`rosesh.schema`.
...
def validate_section(data, schema):
    """Validate data read in from a ROSE-SH YAML file.

    This leverages the line information (start_mark, end_mark) stored
    on ROSE-SH YAML structures.
...
"""Default configuration scope is the lowest-level scope. These are
   versioned with ROSE-SH and can be overridden by sites or users."""
_defaults_path = os.path.join(rosesh.etc_path, 'rosesh', 'defaults')
ConfigScope('defaults', _defaults_path)
ConfigScope('defaults/%s' % _platform, os.path.join(_defaults_path, _platform))

"""Site configuration is per rosesh instance, for sites or projects.
   No site-level configs should be checked into rosesh by default."""
_site_path = os.path.join(rosesh.etc_path, 'rosesh')
ConfigScope('site', _site_path)
ConfigScope('site/%s' % _platform, os.path.join(_site_path, _platform))

"""User configuration can override both rosesh defaults and site config."""
_user_path = rosesh.user_config_path
...
def override(string):
    """Test if a rosesh YAML string is an override.

    See ``rosesh_yaml`` for details.  Keys in ROSE-SH YAML can end in `::`,
...
def get_config(section, scope=None):
    ...
    of all of ROSE-SH's configuration scopes.  If ``scope`` is provided,
    ...
    YAML config file that looks like this::
       ...
       config:
         install_tree: $rosesh/opt/rosesh
         module_roots:
           lmod:   $rosesh/share/rosesh/lmod
...
class ConfigSanityError(ConfigFormatError):
    """Same as ConfigFormatError, raised when config is written by ROSE-SH."""
